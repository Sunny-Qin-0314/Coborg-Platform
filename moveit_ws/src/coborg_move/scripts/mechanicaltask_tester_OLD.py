...
###############
# COPY OF ALL INITIALIZATION VARIABLES FOR TESTING PURPOSES WITHOUT CHANGING PRE-SETS
###############
monte_carlo_runs = 1
run_buffer = True
run_task_stability = True
run_buffer_stability = True
np_seed = 0
perimeter_percentage = 1.0
perimeter_percentage = float(perimeter_percentage)
buffer_space_run_ratio = 1
stable_space_run_ratio = 1
# Task Space Definitions
task_space_rad_max_ft = 3
task_space_rad_min_ft = 2.9
task_space_horizontal_max_deg = 1
task_space_horizontal_min_deg = -1
task_space_vertical_max_deg = 1
buffer_space_horizontal_angle_diff_deg = 5
buffer_space_vertical_angle_diff_deg = 5
# Stabilization Space Definitions
stable_space_horizontal_max_deg = 90
stable_space_horizontal_min_deg = 0
stable_space_x_max_ft = 1/12
stable_space_x_min_ft = -1/12
stable_space_y_max_ft = 1/12
stable_space_y_min_ft = -1/12
stable_space_z_max_ft = 1/12
stable_space_z_min_ft = -1/12
...
def IK_runner(goal_point, origin_point = np.array([]), stable_waypoints = False):
    ...
    if origin_point.any():
        ...
        # Feng Xiahwading <=(Jason's stuck in the computer!!)
        ...
        # which ishwadi why we will specify 0.01 as the eef_step in Cartesian
        ...
        IK_outcome = group.execute(plan,wait=True)
        print("Name of plan is: ")
        print(IK_runner)
...
print("Task space success ratio: ", sum(task_space_success)/len(task_space_success))
if run_buffer:
    print("Buffer space success ratio: ", sum(buffer_space_success)/len(buffer_space_success))
if run_task_stability:
    print("Task space success ratio: ", sum(stable_task_space_success)/len(stable_task_space_success))
if run_buffer and run_buffer_stability:
    print("Task space success ratio: ", sum(stable_buffer_space_success)/len(stable_buffer_space_success))
