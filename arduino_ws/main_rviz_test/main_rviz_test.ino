...
void messageCb(const sensor_msgs::JointState& msg) {
  int angle = msg.position[0]*(180/3.14); //grab data from first joint index (0)then convert rad2deg
  servo.write(angle); // valid inputs: 0-180
}
...
void loop(){
    ...
    if(state == 1){
        digitalWrite(enable_motor, LOW); //turn off DC motor. temp fix.
